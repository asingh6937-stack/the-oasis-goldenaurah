...
// Wix custom TEXT fields reject values longer than 200 characters. Every custom
// field value we send must be clamped to this, or Wix 400s the whole contact create.
const MAX_CUSTOM_FIELD_LEN = 200;
...
// but the cache does not survive across cold starts or concurrent invocations,
// so two near-simultaneous submissions can race to create the same field/label
// name. Wix's find-or-create isn't guaranteed safe under that race and can
// reject the loser with 400. A single retry resolves it: by the time we retry,
// the winner's field/label already exists, so find-or-create simply finds it.
function delay(ms) {
...
async function resolveExtendedFieldKey(cfg, displayName, dataType = 'TEXT') {
  ...
  let data;
  try {
    data = await wixPost('/contacts/v4/extended-fields', { displayName, dataType }, cfg);
  } catch (error) {
    if (error.status !== 400) throw error;
    console.warn(`Wix extended field "${displayName}" got 400, retrying once (likely a create race):`, truncate(JSON.stringify(error.wixResponse || {}), 300));
    await delay(300);
    data = await wixPost('/contacts/v4/extended-fields', { displayName, dataType }, cfg);
  }
...
async function resolveLabelKey(cfg, displayName) {
  ...
  let data;
  try {
    data = await wixPost('/contacts/v4/labels', { displayName }, cfg);
  } catch (error) {
    if (error.status !== 400) throw error;
    console.warn(`Wix label "${displayName}" got 400, retrying once (likely a create race):`, truncate(JSON.stringify(error.wixResponse || {}), 300));
    await delay(300);
    data = await wixPost('/contacts/v4/labels', { displayName }, cfg);
  }
...
// Compact attribution string. Project/Source/Platform are omitted here because
// they are already applied as contact labels. High-value ad params come first so
// that if the string is clamped to the 200-char field limit, the least important
// value (landing page) is what gets cut, not gclid/fbclid.
function attributionSummary(lead) {
  ...
  if (lead.utm_campaign) parts.push(`camp=${lead.utm_campaign}`);
  if (lead.gclid) parts.push(`gclid=${lead.gclid}`);
  if (lead.fbclid) parts.push(`fbclid=${lead.fbclid}`);
  if (lead.utm_content) parts.push(`content=${lead.utm_content}`);
  if (lead.utm_term) parts.push(`term=${lead.utm_term}`);
  if (lead.landing_page) parts.push(`lp=${lead.landing_page}`);
  return parts.join(' | ');
}

// Clamp a custom field value to the Wix 200-char limit so it can never 400 the
// contact create.
function clampCustomField(value) {
  return String(value == null ? '' : value).slice(0, MAX_CUSTOM_FIELD_LEN);
}

async function createWixContact(cfg, baseInfo, labelKeys, extendedItems) {
  const info = { ...baseInfo };
...
async function storeInWix(cfg, lead) {
  ...
  // Best-effort enrichment. Every value is clamped to 200 chars (the Wix custom
  // TEXT field limit). If a field/label can't be resolved (e.g. missing API
  // scope), we omit just that piece.
  const fieldSpecs = [
  ...
  await Promise.all(fieldSpecs.map(async ([displayName, value]) => {
    try {
      ...
      if (key) extendedItems[key] = clampCustomField(value);
    } catch (error) {
  ...
  // Fallback ladder — never let optional enrichment block lead creation.
  // 1) full  2) drop custom fields  3) name + phone only.
  // Only a 400 (payload validation) triggers a downgrade; 401/403 etc. can't be
  // fixed by dropping fields, so they throw immediately.
  const attempts = [
    { label: 'full (labels + custom fields)', labelKeys, extendedItems },
    { label: 'without custom fields', labelKeys, extendedItems: {} },
    { label: 'name + phone only', labelKeys: [], extendedItems: {} },
  ];

  let lastError;
  for (const attempt of attempts) {
    try {
      const contactId = await createWixContact(cfg, baseInfo, attempt.labelKeys, attempt.extendedItems);
      console.log('Wix contact created:', JSON.stringify({
        contactId: contactId || '(no id in response)',
        attempt: attempt.label,
        labelsApplied: attempt.labelKeys.length,
        customFieldsApplied: Object.keys(attempt.extendedItems).length,
      }));
      return contactId;
    } catch (error) {
      lastError = error;
      if (error.status === 400) {
        console.warn(`Contact create returned 400 on "${attempt.label}" attempt; retrying with a smaller payload. Wix said:`, truncate(JSON.stringify(error.wixResponse || {}), 400));
        continue;
      }
      throw error;
    }
