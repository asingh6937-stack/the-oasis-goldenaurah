...
function isAllowedOrigin(origin) {
  ...
  if (origin === 'https://the-oasis-goldenaurah.vercel.app') return true;
...
function corsHeaders(origin) {
  const allowed = allowedOrigins();
  const allowOrigin = isAllowedOrigin(origin) ? (origin || '*') : (allowed[0] || 'null');
  return {
...
async function handleLeadRequest({ method, headers = {}, body, ip }) {
  ...
  console.log('Lead request origin:', origin || '(missing)', 'Allowed origins:', allowedOrigins());
