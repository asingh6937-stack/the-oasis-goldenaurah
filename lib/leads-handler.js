...
const LEADS_TABLE_COLUMNS = new Set([
  'name',
  'phone',
  'budget',
  'plot_interest',
...
function debugEnabled() {
  return ['1', 'true', 'yes', 'on'].includes(env('DEBUG').toLowerCase());
}
...
function getSupabaseServiceRoleKey() {
  ...
  if (key.startsWith('sb_publishable_')) throw new Error('SUPABASE_SERVICE_ROLE_KEY appears to be a publishable key, not a service_role/secret key.');
  ...
  if (parts.length === 3) {
    try {
      ...
      if (payload.role && payload.role !== 'service_role') {
        throw new Error(`SUPABASE_SERVICE_ROLE_KEY JWT role is "${payload.role}", expected "service_role".`);
      }
    } catch (error) {
      if (error.message.includes('expected "service_role"')) throw error;
    }
  }
...
function buildSupabaseLeadPayload(lead) {
  const payload = {};
  const omitted = [];

  Object.entries(lead).forEach(([key, value]) => {
    if (LEADS_TABLE_COLUMNS.has(key)) {
      payload[key] = value;
    } else {
      omitted.push(key);
    }
  ...
  console.log('Supabase lead insert payload keys:', Object.keys(payload));
  if (omitted.length) console.log('Supabase lead insert omitted unknown keys:', omitted);
  return payload;
}
...
async function storeInSupabase(lead) {
  ...
  if (!url) return { skipped: true, provider: 'supabase' };
  const key = getSupabaseServiceRoleKey();
  const endpoint = `${url.replace(/\/$/, '')}/rest/v1/${encodeURIComponent(table)}`;
  const payload = buildSupabaseLeadPayload(lead);
  try {
...
async function handleLeadRequest({ method, headers = {}, body, ip }) {
  ...
  try {
    ...
    if (debugEnabled()) return json(500, debugBody, origin);

    return json(500, {
      ok: false,
      error: 'Lead could not be saved. Set DEBUG=true to return full storage error details.',
    }, origin);
  }
}
